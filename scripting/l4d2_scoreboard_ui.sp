...
public void OnMapStart()
{
    RemoveAllHUD();
    EnableHUD();
    // 贴左上极限: X 0.01->0.005, Y 标题 0.008->0.002 列 0.028->0.015 紧贴, 总宽 0.33
    HUDPlace(COL_TITLE, 0.005,0.002, 0.35, 0.018);
    HUDPlace(COL_RANK,  0.005,0.015, 0.025, 0.16);
    HUDPlace(COL_NAME,  0.030,0.015, 0.04, 0.16);
    HUDPlace(COL_SCORE, 0.070,0.015, 0.06, 0.16);
    HUDPlace(COL_SI,    0.130,0.015, 0.04, 0.16);
    HUDPlace(COL_KILL,  0.170,0.015, 0.04, 0.16);
    HUDPlace(COL_FF,    0.210,0.015, 0.05, 0.16);
    HUDPlace(COL_BLK,   0.260,0.015, 0.05, 0.16);
    g_bHudReady = true;
    if (g_hTimer == null)
