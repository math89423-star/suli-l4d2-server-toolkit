...
public Action:tmrIntro(Handle:timer, any:client)
{
	...
	if( client && IsClientInGame(client) && GetClientTeam(client) == 2 )
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Intro", client);
}
...
public Action:tmrFlareHintMsg(Handle:timer, any:client)
{
	...
	{
		if( g_iGrndCmdAllow )
			PrintToChat(client, "%s%T", TAG_CHAT, "Flare Cmd Ground", client);
		else if( g_iSelfCmdAllow )
			PrintToChat(client, "%s%T", TAG_CHAT, "Flare Cmd Attach", client);		
	}
}
...
CommandForceFlare(client, target, args, const String:sArg[], bool:bGroundFlare)
{
	...
	if( !IsFlareValidNow() )
	{
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Invalid Now", client);
		return;
	}
	...
	{
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Invalid Target", client);
		return;
	}
	...
	{
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Invalid Args", client);
		return;
	}
	...
	{
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Max", client);
		return;
	}
	...
	{
		PrintToChat(client, "%s%T", TAG_CHAT, "Flare Wait", client);
		return;
	}
...
CmdCreateFlare(client, args, const String:sArg[], bool:bGroundFlare)
{
	...
	{
		PrintToChat(client, "[SM] %T.", "No Access", client);
		return;
	}
	...
	else
	{
		...
		{
			if( g_bHint )
				PrintToChat(client, "%s%T", TAG_CHAT, "Flare Incapped", client);
			return;
		}
	}
	...
	if( args != 0 && args != 3 && args != 6 )
	{
		...
		if( g_bHint )
			PrintToChat(client, "%s%T", TAG_CHAT, "Flare Usage", client);
		return;
	}
...
CreateFlare(client, const String:sColourL[], const String:sColourS[]="", bool:bGroundFlare=false)
{
	...
	if( bGroundFlare )
	{
		...
		{
			PrintToChat(client, "%s%T", TAG_CHAT, "Flare Invalid Place", client);
...
FlareGun(client)
{
	if( g_bBounce[client] )
	{
		...
		PrintToChat(client, "%sYou've disabled Flare Gun bounces.", TAG_CHAT);
	}
	else
	{
		SetClientCookie(client, g_hCookie, "1");
		PrintToChat(client, "%sYou've enabled Flare Gun bounces.", TAG_CHAT);
	}
...
public Action:OnWeaponEquip(client, weapon)
{
	if( !g_bLeft4Dead && g_bGunAllow && g_iGunBounce == 1 && IsFlareValidNow() && !g_bDisplayed[client] && GetClientTeam(client) == 2 )
	{
		...
		{
			PrintToChat(client, "%s%T", TAG_CHAT, "Flare Gun Bounce", client);
