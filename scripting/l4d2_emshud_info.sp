...
void IsKillStatistics()
{
	...
	char sData[128], sInfo[sizeof(g_sName)][32];
	int iDistance[sizeof(g_sName)], iValue[sizeof(g_sName)];
	...
	for (int i = 0; i < sizeof(iDistance); i++)
		if (iDistance[i] > 0)
			FormatEx(sInfo[i], sizeof(sInfo[]), "%s%s%d%s\n", g_sName[i][0], GetAddSpacesMax(iValue[i], "0"), iDistance[i], g_sName[i][1]);
	...
	HUDPlace(HUD_FAR_LEFT,0.00,0.28,1.0,0.22);
...
void IsKillLeaderboards()
{
	...
	for (int x = 0; x < assister_count+1; x++)
	{
		int j = x + 1;
...
void IsShowServersNumber(int[] iQuantity)
{
	...
	if (iDistance[0] > iDistance[1])//如果当前路程大于刷坦克路程.
		iDistance[1] = 0;//重新赋值.

	if (iDistance[0] > iDistance[2])//如果当前路程大于刷女巫路程.
		iDistance[2] = 0;
	...
	HUDPlace(HUD_SCORE_1,0.00,g_iShowServerName <= -1?0.035:0.06,1.0,0.25);
}
//获取玩家总数量.
