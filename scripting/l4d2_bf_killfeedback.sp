/**
 * [L4D2] Battlefield Kill Feedback  v3.5.1
 *
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (victim >= 1 && victim <= MaxClients && GetClientTeam(victim) == 3)
    {
        ...
        if (validAttacker)
        {
            ...
            LogMessage("[BF] Event_PlayerDeath: victim=%N attacker=%N headshot=%d melee=%d sound='%s'",
                victim, attacker, headshot, melee, sound);
        ...
        else
        {
            ...
            if (g_cvCenterTextEnabled.BoolValue)
            {
                char centerMsg[128];
                if (suicide)
                    Format(centerMsg, sizeof(centerMsg), "%s  自杀了", siName);
                else
                    Format(centerMsg, sizeof(centerMsg), "%s  死于意外", siName);

                for (int i = 1; i <= MaxClients; i++)
                {
                    if (IsClientInGame(i) && GetClientTeam(i) == 2)
                        PrintCenterText(i, centerMsg);
                }
            }
        }
        return Plugin_Continue;
                     ...
                     bool isTank, bool isWitch)
{
    ...
    // Center text via PrintCenterText — separate channel from HintText,
    // so L4D_All_Infected_HUD_HP (which uses PrintHintText) never touches it.
    LogMessage("[BF] ShowKillDisplay: client=%N cvCenterText=%d", client, g_cvCenterTextEnabled.BoolValue);
    if (g_cvCenterTextEnabled.BoolValue)
    {
        char centerMsg[128];
        Format(centerMsg, sizeof(centerMsg), "[%s] KILL %s%s",
            weaponDisplay, siName, suffix);
        LogMessage("[BF] PrintCenterText(%N, \"%s\")", client, centerMsg);
        PrintCenterText(client, centerMsg);
        LogMessage("[BF] PrintCenterText done, creating 2s clear timer");
        CreateTimer(2.0, Timer_ClearCenterText, GetClientUserId(client),
...
{
    LogMessage("[BF] PlayClientSound ENTER: client=%N sound='%s'", client, sound);
    if (sound[0] == '\0') { LogMessage("[BF] PlayClientSound EXIT: empty sound path"); return; }
    float vol = g_cvVolume.FloatValue;
    if (vol <= 0.0) { LogMessage("[BF] PlayClientSound EXIT: vol=%.2f", vol); return; }
    // Strip file extension — Source engine sound lookup uses bare name
    char name[PLATFORM_MAX_PATH];
    strcopy(name, sizeof(name), sound);
    int len = strlen(name);
    if (len > 4 && name[len-4] == '.') name[len-4] = '\0';
    // Emit from world entity (0) so the engine doesn't cull it as
    // "local player's own sound".  SNDCHAN_STATIC = non-spatialized
    // UI channel — attacker hears it at full volume regardless of position.
    EmitSoundToClient(client, name, 0, SNDCHAN_STATIC, SNDLEVEL_NORMAL,
        SND_NOFLAGS, vol >= 1.0 ? 1.0 : vol);
    LogMessage("[BF] EmitSoundToClient(%N, %s (raw=%s), entity=world, chan=static, vol=%.2f)",
        client, name, sound, vol);
}
