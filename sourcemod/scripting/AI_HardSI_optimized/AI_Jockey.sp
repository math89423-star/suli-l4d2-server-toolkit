...
public Action:Jockey_OnPlayerRunCmd(jockey, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon, bool:hasBeenShoved) {
	...
	new bool:bHasLOS = bool:GetEntProp(jockey, Prop_Send, "m_hasVisibleThreats");
...
public Action:Jockey_OnSpawn(botJockey) {
	...
	bDoNormalJump[botJockey] = false;  // FIXED: init jump state
...
// FIXED: Added client validity check
public Action:Timer_LeapCooldown(Handle:timer, any:jockey) {
	if ( IsValidClient(jockey) && IsBotJockey(jockey) && IsPlayerAlive(jockey) ) {
