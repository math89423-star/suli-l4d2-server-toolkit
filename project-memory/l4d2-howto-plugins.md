...
python3 /tmp/rcon.py 'sm plugins reload 插件名'
```

> **RCON 客户端**: `/tmp/rcon.py`，端口 27015，密码在 `server.cfg`。
...
```bash
# 列出所有插件
python3 /tmp/rcon.py 'sm plugins list'

# 查看特定插件
python3 /tmp/rcon.py 'sm plugins info 插件名'
...
```bash
# 确认插件加载无错误
python3 /tmp/rcon.py 'sm plugins list' 2>&1 | grep "插件名"

# 确认 cvar 生效
python3 /tmp/rcon.py 'sm_cvar cvar名' 2>&1 | grep "Value"
```
